...
describe("Pokedex Component", () => {
  it("can be rendered", () => {
    expect(<Pokedex fetchAllPokemons={jest.fn()} />).toBeDefined();
  });

  it("it has a title", () => {
    const wrapper = shallow(<Pokedex fetchAllPokemons={jest.fn()} />);
    expect(wrapper.find("h1").text()).toEqual("Pokedex");
  });

  it("user can click to go to Create Page", () => {
    const wrapper = shallow(
      <Pokedex fetchAllPokemons={jest.fn()} link={jest.fn()} />
    );
